Alias: $tre-r397-categorie-entite-geographique-exercice = https://smt.esante.gouv.fr/fhir/CodeSystem/tre-r397-categorie-entite-geographique-exercice

ValueSet: JdvJ307CategorieEntiteGeographiqueExerciceNiv1Finess
Id: jdv-j307-categorie-entite-geographique-exercice-niv1-finess
Title: "Jdv J307 Categorie Entite Geographique Exercice Niv1 Finess"
Description: "Jeu de valeurs FINESS contenant les catégories des entités géographiques d'exercice de niveau 1"
* ^meta.versionId = "1"
* ^meta.lastUpdated = "2025-11-04T18:02:28.249+00:00"
* ^meta.profile = "http://hl7.org/fhir/StructureDefinition/shareablevalueset"
* ^language = #fr-FR
* ^extension.url = "http://hl7.org/fhir/StructureDefinition/resource-effectivePeriod"
* ^extension.valuePeriod.start = "2025-11-04T18:02:28.249+00:00"
* ^url = "https://smt.esante.gouv.fr/fhir/ValueSet/jdv-j307-categorie-entite-geographique-exercice-niv1-finess"
* ^identifier.system = "urn:ietf:rfc:3986"
* ^identifier.value = "urn:oid:1.2.250.1.213.1.6.1.315"
* ^version = "20251104120000"
* ^status = #active
* ^experimental = false
* ^date = "2025-11-04T18:02:28.249+00:00"
* ^publisher = "Agence du Numérique en Santé (ANS) - 2-10 Rue d'Oradour-sur-Glane, 75015 Paris"
* include codes from system $tre-r397-categorie-entite-geographique-exercice where niveau = "1"