Alias: $tre-r376-type-evenement-structure = https://smt.esante.gouv.fr/fhir/CodeSystem/tre-r376-type-evenement-structure

ValueSet: JdvJ297TypeEvenementAdministratifFiness
Id: jdv-j297-type-evenement-administratif-finess
Title: "Jdv J297 Type Evenement Administratif Finess"
Description: "Jeu de valeurs FINESS contenant les événements affectant le cycle de vie des entités et activités enrgeistrées dans FINESS+"
* ^meta.versionId = "1"
* ^meta.lastUpdated = "2025-06-11T18:02:28.249+00:00"
* ^meta.profile = "http://hl7.org/fhir/StructureDefinition/shareablevalueset"
* ^language = #fr-FR
* ^extension.url = "http://hl7.org/fhir/StructureDefinition/resource-effectivePeriod"
* ^extension.valuePeriod.start = "2025-06-11T18:02:28.249+00:00"
* ^url = "https://smt.esante.gouv.fr/fhir/ValueSet/jdv-j297-type-evenement-administratif-finess"
* ^identifier.system = "urn:ietf:rfc:3986"
* ^identifier.value = "urn:oid:1.2.250.1.213.1.6.1.304"
* ^version = "20250611120000"
* ^status = #active
* ^experimental = false
* ^date = "2025-06-11T18:02:28.249+00:00"
* ^publisher = "Agence du Numérique en Santé (ANS) - 2-10 Rue d'Oradour-sur-Glane, 75015 Paris"
* include codes from system $tre-r376-type-evenement-structure where finess = "true"
