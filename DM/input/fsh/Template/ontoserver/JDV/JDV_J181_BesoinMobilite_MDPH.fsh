ValueSet: JDV_J181_BesoinMobilite_MDPH
Id: JDV-J181-BesoinMobilite-MDPH
Description: "Caractérise les besoins d'aide de la personne en matière de scolarité, en lien avec la mobilité"
* ^meta.versionId = "4"
* ^meta.lastUpdated = "2025-07-02T17:05:33.887+00:00"
* ^meta.profile = "http://hl7.org/fhir/StructureDefinition/shareablevalueset"
* ^language = #fr-FR
* ^extension.url = "http://hl7.org/fhir/StructureDefinition/resource-effectivePeriod"
* ^extension.valuePeriod.start = "2021-09-24T12:00:00+01:00"
* ^url = "https://mos.esante.gouv.fr/NOS/JDV_J181-BesoinMobilite-MDPH/FHIR/JDV-J181-BesoinMobilite-MDPH"
* ^identifier.system = "urn:ietf:rfc:3986"
* ^identifier.value = "urn:oid:1.2.250.1.213.3.4.16"
* ^version = "20210924120000"
* ^status = #active
* ^experimental = false
* ^date = "2021-09-24T12:00:00+01:00"
* ^publisher = "Agence du Numérique en Santé (ANS) - 2-10 Rue d'Oradour-sur-Glane, 75015 Paris"
* ^jurisdiction = urn:iso:std:iso:3166#FR
* $TRE-R322-BesoinMobilite#01 "Pour se déplacer à l'extérieur des locaux"
* $TRE-R322-BesoinMobilite#02 "Pour se déplacer à l'intérieur des locaux"
* $TRE-R322-BesoinMobilite#03 "Pour utiliser les transports en commun"
* $TRE-R302-ContexteCodeComplementaire#02 "Autre"