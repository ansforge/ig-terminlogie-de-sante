ValueSet: JDV_J193_TypeSourceCapacite_ROR
Id: JDV-J193-TypeSourceCapacite-ROR
Description: "Permet d'indiquer dans le ROR quel type de source est à l'origine de l'information de capacité"
* ^meta.versionId = "3"
* ^meta.lastUpdated = "2024-04-27T05:07:11.744+00:00"
* ^meta.profile = "http://hl7.org/fhir/StructureDefinition/shareablevalueset"
* ^extension.url = "http://hl7.org/fhir/StructureDefinition/resource-effectivePeriod"
* ^extension.valuePeriod.start = "2022-05-27T12:00:00+01:00"
* ^url = "https://mos.esante.gouv.fr/NOS/JDV_J193-TypeSourceCapacite-ROR/FHIR/JDV-J193-TypeSourceCapacite-ROR"
* ^identifier.system = "urn:ietf:rfc:3986"
* ^identifier.value = "urn:oid:1.2.250.1.213.3.4.26"
* ^version = "20220527120000"
* ^status = #active
* ^experimental = false
* ^date = "2022-05-27T12:00:00+01:00"
* ^publisher = "Agence du Numérique en Santé (ANS) - 2-10 Rue d'Oradour-sur-Glane, 75015 Paris"
* $TRE-R335-TypeSourceCapacite#01 "Saisie manuelle"
* $TRE-R335-TypeSourceCapacite#02 "Bed management"
* $TRE-R335-TypeSourceCapacite#03 "Gestion administrative du malade (GAM)"
* $TRE-R335-TypeSourceCapacite#04 "Dossier patient informatisé (DPI)"